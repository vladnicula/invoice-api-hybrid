...
export class ClientsService {
  ...
  async findByUserId(
    ...
    const clientIdsOfThisUserIdQuery = this.clientsRepository
    ...
    const invoiceTotalByClientIdQuery = this.invoicesRepository
      ...
      .where(
        `invoiceTotals.clientId IN (${clientIdsOfThisUserIdQuery.getQuery()})`,
      )
      .groupBy('clientId');
    ...
    if (sortBy === 'name' || sortBy === 'contactName') {
      clientListQuery = clientListQuery.orderBy(
        `client.${sortBy}`,
        sort ?? 'ASC',
      );
    }
    ...
    if (skip !== undefined) {
      clientListQuery = clientListQuery.offset(skip);
    }

    if (limit !== undefined) {
      clientListQuery = clientListQuery.limit(limit);
    }
