...
import { Args, Info, Int, Parent, Query, ResolveField, Resolver } from '@nestjs/graphql';
...
export class ClientInvoicesAggregateResolver {
  ...
  async invoices(
    ...
    const invoiceSelectFields = Object.keys(queryFields(queryInfo)) as (keyof InvoiceEntity)[]
    const invoices = await this.invoicesService.findByUserId(user.id, {
      limit, skip: 0, sort, sortBy, startDate, endDate, clientId: parent.id, selectFields: invoiceSelectFields
    });
